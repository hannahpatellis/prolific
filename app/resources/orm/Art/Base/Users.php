...
abstract class Users implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Art\\Map\\UsersTableMap';
    ...
    /**
     * The value for the selectiononly field.
     *
     * Note: this column has a database default value of: true
     * @var        boolean
     */
    protected $selectiononly;
    ...
    /**
     * Initializes internal state of Art\Base\Users object.
     * @see applyDefaults()
     */
    ...
    /**
     * Get the [selectiononly] column value.
     *
     * @return boolean
     */
    ...
    public function setIsadmin($v)
    {
        ...
        if ($this->isadmin !== $v) {
            $this->isadmin = $v;
            $this->modifiedColumns[UsersTableMap::COL_ISADMIN] = true;
        }
    ...
    /**
     * Sets the value of the [selectiononly] column.
     * Non-boolean arguments are converted using the following rules:
     *   * 1, '1', 'true',  'on',  and 'yes' are converted to boolean true
     *   * 0, '0', 'false', 'off', and 'no'  are converted to boolean false
     * Check on string values is case insensitive (so 'FaLsE' is seen as 'false').
     ...
     * @return $this The current object (for fluent API support)
     */
    public function setSelectiononly($v)
    {
        if ($v !== null) {
            if (is_string($v)) {
                $v = in_array(strtolower($v), array('false', 'off', '-', 'no', 'n', '0', '')) ? false : true;
        ...
        if ($this->selectiononly !== $v) {
            $this->selectiononly = $v;
            $this->modifiedColumns[UsersTableMap::COL_SELECTIONONLY] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : UsersTableMap::translateFieldName('Isadmin', TableMap::TYPE_PHPNAME, $indexType)];
            $this->isadmin = (null !== $col) ? (boolean) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : UsersTableMap::translateFieldName('Selectiononly', TableMap::TYPE_PHPNAME, $indexType)];
            $this->selectiononly = (null !== $col) ? (boolean) $col : null;
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Art\\Users'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(UsersTableMap::COL_ISADMIN)) {
            $modifiedColumns[':p' . $index++]  = 'isAdmin';
        }
        if ($this->isColumnModified(UsersTableMap::COL_SELECTIONONLY)) {
            $modifiedColumns[':p' . $index++]  = 'selectionOnly';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'isAdmin':
                        ...
                        break;
                    case 'selectionOnly':
                        $stmt->bindValue($identifier, (int) $this->selectiononly, PDO::PARAM_INT);
    ...
    public function fromArray(array $arr, string $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[7], $arr)) {
            $this->setSelectiononly($arr[$keys[7]]);
        }
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(UsersTableMap::COL_ISADMIN)) {
            $criteria->add(UsersTableMap::COL_ISADMIN, $this->isadmin);
        }
        if ($this->isColumnModified(UsersTableMap::COL_SELECTIONONLY)) {
            $criteria->add(UsersTableMap::COL_SELECTIONONLY, $this->selectiononly);
        }
        if ($this->isColumnModified(UsersTableMap::COL_CREATED_AT)) {
     ...
     * objects.
     *
     * @param object $copyObj An object of \Art\Users (or compatible) type.
    ...
    public function copyInto(object $copyObj, bool $deepCopy = false, bool $makeNew = true): void
    {
        ...
        $copyObj->setSelectiononly($this->getSelectiononly());
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Art\Users Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
