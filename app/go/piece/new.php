...
<?php if(isset($_GET['status']) && $_GET['status'] == '500') { ?>
  <div class="row">
    <div class="col">
      <div class="alert alert-danger" role="alert">There was an error adding the new EMOH<?php if($_GET['detail']) {print(": " . htmlspecialchars($_GET['detail'], ENT_QUOTES, 'UTF-8'));}; ?></div>
    </div>
  </div>
<?php } ?>

<form action="/api/piece/new.php" method="POST" enctype="multipart/form-data">
    ...
    <div class="col">
      <div class="form-floating">
        <select class="form-select" id="form-collection" name="collection" aria-label="Collection" required>
          ...
          <option value="Upscaled26" selected>Upscaled26</option>
