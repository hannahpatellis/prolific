...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $new_cfa = new Art\Cfa();
  $new_cfa->fromArray($prepared_properties);
  $new_cfa->setUpdatedAt('now');
  $new_cfa->save();
  header("Location: /go/cfa/new.php?status=201");
} else {
  header("Location: /go/cfa/new.php?status=500&detail=notPOST");
}
